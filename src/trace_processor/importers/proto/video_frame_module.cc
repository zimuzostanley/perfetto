...
void VideoFrameModule::ParseTracePacketData(const TracePacket::Decoder& decoder,
  ...
  // Store JPEG image data in the blob vector, indexed by row number.
  if (frame.has_jpg_image()) {
    auto jpg = frame.jpg_image();
    auto blob = TraceBlob::Allocate(jpg.size);
    memcpy(blob.data(), jpg.data, jpg.size);
    auto* blob_vec = context_->storage->mutable_video_frame_data();
    if (blob_vec->size() <= row_idx) {
      blob_vec->resize(row_idx + 1);
    }
    (*blob_vec)[row_idx] = TraceBlobView(std::move(blob), 0, jpg.size);
  }
}
