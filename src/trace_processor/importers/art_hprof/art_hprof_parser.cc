...
void ArtHprofParser::PopulateFieldValues(const HeapGraph& graph) {
  ...
  for (auto it = graph.GetObjects().GetIterator(); it; ++it) {
    ...
    // Set value_string for objects that have decoded strings (String objects
    // and primitive arrays with generated text representations).
    if (obj.GetDecodedString().has_value()) {
      StringId str_id =
          context_->storage->InternString(*obj.GetDecodedString());
      object_table.FindById(*owner_table_id)->set_value_string(str_id);
    }
    ...
    if (obj.GetObjectType() == ObjectType::kInstance ||
      ...
      for (const auto& field : obj.GetFields()) {
        if (field.GetType() == FieldType::kObject) {
          // Check if target is a java.lang.String with a decoded value.
    ...
    if (!has_value_fields && !has_array_data)
      continue;

    uint32_t field_set_id = 0;
    if (has_value_fields) {
      field_set_id = static_cast<uint32_t>(field_table.row_count());
      object_table.FindById(*owner_table_id)->set_field_set_id(field_set_id);
    }
    ...
    if (has_value_fields) {
      ...
      for (const auto& field : obj.GetFields()) {
        // Include kObject fields only if they point to a java.lang.String.
        if (field.GetType() == FieldType::kObject) {
          auto target_id = field.GetValue<uint64_t>();
          if (!target_id || *target_id == 0)
            continue;
    ...
    if (has_array_data) {
          ...
          [&](const auto& vec) {
            ...
            if constexpr (!std::is_same_v<T, std::monostate>) {
              ...
              if constexpr (std::is_same_v<ElemT, bool>) {
                // std::vector<bool> is bit-packed; serialize manually.
                blob.resize(vec.size());
                for (size_t i = 0; i < vec.size(); ++i)
                  blob[i] = vec[i] ? 1 : 0;
              } else {
                // All other primitive types: memcpy from the typed vector
                // which is already in native (little-endian) byte order.
      ...
      if (!blob.empty()) {
        ...
        auto row_ref = object_table.FindById(*owner_table_id);
        row_ref->set_array_element_type(type_id);
        row_ref->set_array_element_count(element_count);
        row_ref->set_array_data_id(blob_id);
      }
    }
  }
}
