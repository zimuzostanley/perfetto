...
function buildDiffMetric(
  ...
  const statement = `
    ...
    SELECT
      ...
      CASE
        WHEN j.is_new = 1 THEN 'NEW'
        WHEN s.m IS NULL OR s.m = 0 OR j.delta_${dim} = 0 THEN 'UNCHANGED'
        WHEN j.delta_${dim} > 0 THEN 'GREW'
        ELSE 'SHRANK'
      END AS diff_direction,
      CASE
        WHEN s.m IS NULL OR s.m = 0 THEN 0.0
        ELSE abs(j.delta_${dim}) * 1.0 / s.m
      END AS diff_intensity
