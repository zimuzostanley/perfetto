...
export function aggregateSharedMappings(
  ...
  const byName = new Map<
    string,
    {pids: Set<number>; pss: number; rss: number; size: number}
  >();
  for (const [pid, aggregated] of smapsData) {
    ...
    for (const g of aggregated) {
      ...
      if (existing !== undefined) {
        ...
        existing.pss += g.pssKb;
        existing.rss += g.rssKb;
        existing.size += g.sizeKb;
      } else {
        byName.set(g.name, {
          pids: new Set([pid]),
          pss: g.pssKb,
  ...
  const result: SharedMapping[] = [];
  for (const [name, data] of byName) {
    if (data.pids.size < 2) continue;
    result.push({
      name,
...
export function parseLruProcesses(output: string): ProcessInfo[] {
  ...
  for (const line of output.split('\n')) {
    // Match lines like: "  0: fore  /1234 (com.android.systemui)"
