...
def build_app_graph(g, scale):
    ...
    # ---- UI subtree -------------------------------------------------------
    am = g.new(CLS['android.app.ActivityManager']);          g.link(app, am)
    for _ in range(scale['activities']):
        act = g.new(CLS['android.app.Activity']);            g.link(am, act)
        fm = g.new(CLS['android.app.FragmentManager']);      g.link(act, fm)
        for _ in range(scale['fragments_per_activity']):
            frag = g.new(CLS['android.app.Fragment']);       g.link(fm, frag)
            for _ in range(scale['holders_per_fragment']):
                vh = g.new(CLS['android.view.ViewHolder']);  g.link(frag, vh)
                for _ in range(scale['textviews_per_holder']):
                    tv = g.new(CLS['android.widget.TextView']);     g.link(vh, tv)
                    for _ in range(scale['strings_per_textview']):
                        s = g.new(CLS['java.lang.String']);  g.link(tv, s)
                for _ in range(scale['imageviews_per_holder']):
                    iv = g.new(CLS['android.widget.ImageView']);    g.link(vh, iv)
                    for _ in range(scale['bitmaps_per_imageview']):
                        b = g.new(CLS['android.graphics.Bitmap']);  g.link(iv, b)
            vm = g.new(CLS['androidx.lifecycle.ViewModel']); g.link(frag, vm)
            for _ in range(scale['caches_per_viewmodel']):
                dc = g.new(CLS['com.example.DataCache']);    g.link(vm, dc)
                for _ in range(scale['entries_per_cache']):
                    ce = g.new(CLS['com.example.CacheEntry']); g.link(dc, ce)
                    ba = g.new(CLS['byte[]'], self_size=128); g.link(ce, ba)
            for _ in range(scale['apis_per_viewmodel']):
                api = g.new(CLS['com.example.ApiClient']);   g.link(vm, api)
                for _ in range(scale['resps_per_api']):
                    r = g.new(CLS['com.example.HttpResponse']); g.link(api, r)
                    ba = g.new(CLS['byte[]'], self_size=512); g.link(r, ba)
        bs = g.new(CLS['android.app.BackStack']);            g.link(fm, bs)
        for _ in range(scale['backstack_entries']):
            bse = g.new(CLS['android.app.BackStackEntry']); g.link(bs, bse)

    # ---- Service subtree -------------------------------------------------
    sm = g.new(CLS['android.app.ServiceManager']);           g.link(app, sm)
    for _ in range(scale['bgservices']):
        svc = g.new(CLS['com.example.BackgroundService']);   g.link(sm, svc)
        for _ in range(scale['workers_per_service']):
            w = g.new(CLS['com.example.Worker']);            g.link(svc, w)
            for _ in range(scale['tasks_per_worker']):
                t = g.new(CLS['com.example.Task']);          g.link(w, t)
                td = g.new(CLS['com.example.TaskData']);     g.link(t, td)
                ba = g.new(CLS['byte[]'], self_size=64);     g.link(td, ba)
            sched = g.new(CLS['com.example.Scheduler']);     g.link(w, sched)
            for _ in range(scale['scheduled_per_scheduler']):
                st = g.new(CLS['com.example.ScheduledTask']);g.link(sched, st)
        eb = g.new(CLS['com.example.EventBus']);             g.link(svc, eb)
        for _ in range(scale['listeners_per_eventbus']):
            el = g.new(CLS['com.example.EventListener']);    g.link(eb, el)
    cs = g.new(CLS['android.net.ConnectivityService']);      g.link(sm, cs)
    cp = g.new(CLS['android.net.ConnectionPool']);           g.link(cs, cp)
    for _ in range(scale['connections']):
        c = g.new(CLS['android.net.Connection']);            g.link(cp, c)
        for _ in range(scale['buffers_per_connection']):
            sb = g.new(CLS['android.net.SocketBuffer']);     g.link(c, sb)
...
def main():
    ...
    with open(out_path, 'w') as f:
        f.write(txt)
    print(f'wrote {out_path} ({len(txt)} bytes, {len(g1.objects)+len(g2.objects)} objects)')
